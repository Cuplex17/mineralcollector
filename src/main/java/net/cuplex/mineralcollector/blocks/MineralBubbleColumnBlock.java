...
public class MineralBubbleColumnBlock extends Block implements FluidDrainable
{
    public static final BooleanProperty DRAG;

    public MineralBubbleColumnBlock(Settings settings)
    {
        super(settings);
    ...
    public static void update(IWorld iWorld_1, BlockPos blockPos_1, boolean boolean_1) {
        if (isStillWater(iWorld_1, blockPos_1)) {
            iWorld_1.setBlockState(blockPos_1, (BlockState) MineralCollector.MINERAL_BUBBLE_COLUMN_BLOCK.getDefaultState().with(DRAG, boolean_1), 2);
        }
    ...
    public static boolean isStillWater(IWorld iWorld_1, BlockPos blockPos_1) {
        ...
        return iWorld_1.getBlockState(blockPos_1).getBlock() == Blocks.WATER && fluidState_1.getLevel() >= 8 && fluidState_1.isStill();
    }
    ...
    private static boolean calculateDrag(BlockView blockView_1, BlockPos blockPos_1) {
        ...
        if (block_1 == MineralCollector.MINERAL_BUBBLE_COLUMN_BLOCK) {
            return (Boolean) blockState_1.get(DRAG);
        } else {
            return block_1 == Blocks.SOUL_SAND;
        }
    }
    ...
    public void randomDisplayTick(BlockState blockState_1, World world_1, BlockPos blockPos_1, Random random_1) {
        ...
        if ((Boolean)blockState_1.get(DRAG)) {
            ...
            world_1.addImportantParticle(ParticleTypes.LARGE_SMOKE, double_1 + 0.5D, double_2, double_3 + 0.5D, 0.0D, 0.04D, 0.0D);
    ...
    public BlockState getStateForNeighborUpdate(BlockState blockState_1, Direction direction_1, BlockState blockState_2, IWorld iWorld_1, BlockPos blockPos_1, BlockPos blockPos_2) {
        if (!blockState_1.canPlaceAt(iWorld_1, blockPos_1)) {
            ...
            if (direction_1 == Direction.DOWN) {
                iWorld_1.setBlockState(blockPos_1, (BlockState)MineralCollector.MINERAL_BUBBLE_COLUMN_BLOCK.getDefaultState().with(DRAG, calculateDrag(iWorld_1, blockPos_2)), 2);
            } else if (direction_1 == Direction.UP && blockState_2.getBlock() != MineralCollector.MINERAL_BUBBLE_COLUMN_BLOCK && isStillWater(iWorld_1, blockPos_2)) {
    ...
    public boolean canPlaceAt(BlockState blockState_1, ViewableWorld viewableWorld_1, BlockPos blockPos_1) {
        ...
        return block_1 == MineralCollector.MINERAL_BUBBLE_COLUMN_BLOCK || block_1 == MineralCollector.MINERAL_VENT_BLOCK;
    }
